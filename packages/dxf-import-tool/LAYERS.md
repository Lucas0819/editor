...
- 当前 `dxf-to-scene` **不按图层名拆分楼层**；图层名用于 **`mapDxfLayerToPascal`** / `--mapping-file` 的**类型**（墙、门窗等）。多 `Level` 需在后续管线或扩展中实现。
- 管线主要按 **LINE/LWPOLYLINE + 图层** 出墙；**专业语义**（消防、暖通设备层）需在上层业务或后续 JSON 里扩展，本文档仅约定 **命名与解析规则**。
...
2. 判断是否存在 **xref 式前缀**；决定用「全名」还是「去前缀后缀」做 **图层映射**。
3. 确认 **系统层** 是否参与导出。
4. 若需多 `Level`，在业务层约定 **标高 / 分带 / 分文件** 等策略（本工具默认单层）。
