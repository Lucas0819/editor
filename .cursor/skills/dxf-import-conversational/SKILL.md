...
description: >-
  引导用户完成 DXF→Pascal 场景 JSON：预读（dxf-preview.sh）、可选 dxf-text-search
  按关键词取楼层 TEXT 坐标、确认 mapping 与 CLI、执行 dxf-to-scene；生成 JSON 后询问是否
  浏览器预览，按需检测 3002 端口并启动 bun dev。适用于用户提供 .dxf、dxf-import-tool、
  mapping、图层预处理、floorPlan、或「确认后再转换 / 预览」时。
---
...
- 可重复 `--keyword`（命中**任一**子串即收录）；可重复 `--layer`（**任一**图层匹配）；`--max-matches` 限制条数（默认 10000）。  
